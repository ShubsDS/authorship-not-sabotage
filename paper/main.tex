...
\newcommand{\monStoredHeadAbs}{0.958}   % = \monStoredHead to 3 dp, ABSTRACT ONLY
...
programming problem, an honest solution beside a solution carrying a planted backdoor. In the
corpora used for this, the honest solution is human-written and the backdoor is written by a
language model, so the two classes differ in who wrote them as well as in whether they are
sabotaged. Filtering to working solutions and stripping every comment does not remove that
difference. On the filtered, comment-stripped split, a bag-of-words classifier separates the two
classes at AUROC \tfidfKoran{}, and a classifier that reads no code at all, only problem
metadata, reaches \nocodeCV{}. The signal transfers across backdoor generators and is almost as
strong on backdoors that never work, so it is not a property of the sabotage. We then rebuild the
corpus so that one model writes both classes on the same problems, and release it. Holding the
writer constant removes between a third and nine-tenths of the separability, and holding the
prompt constant as well removes most of the rest. An LLM monitor moves the same way, from
\monStoredHeadAbs{} on stored pairs to \monitorS{} on same-generator pairs. The label installs
a second artifact: a problem whose backdoor fails contributes its human honest solution, so
the negative class records attack failure. Stored-pair evaluations
should report a no-code and a bag-of-words floor against permutation nulls, generate both
classes from one model under one prompt, and define the negative class by honesty rather than by
attack failure.
\end{abstract}
...
There are various known confounding variables between the human and AI solutions, such as in commenting style, so corpora are often preprocessed to mitigate them. The filtered
release keeps only working honest solutions and working backdoors and strips every comment
\cite{koran2026ensemble}; Control Tax reports its pass-tests pre-filter and moves its failed
backdoors to an appendix \cite{terekhov2025controltax}; the monitor-ensemble harness of
\cite{jha2026decorrelation} ships a \texttt{--leak-check} flag with two trivial baselines in
it: a \texttt{\#}-count detector that falls from \hashRaw{} to \hashStripped{} once comments
are stripped, and code length alone, at \lenFiltered{}. Outside AI control, bag-of-words is known to separate human-written from
model-written code \cite{orel2026aicd}, and the benchmarks built for that task match their two
classes on line and character count or on shared problem definitions \cite{guo2025codemirage,
demirok2025multiaigcd}. Adjacent control work attacks other axes: monitors subverted with
injected text, attacker-side selection shown to inflate safety estimates, transcript length
removed as a nuisance covariate \cite{terekhov2026adaptive, gewang2026attackselection,
...
\monN{} open-weight monitors ranked on that same split, whose median score is \monMedian{}; a
classifier that reads no code at all, only problem difficulty, test count and question length
-- the \emph{no-code floor} -- reaches \nocodeCV{}, above \nocodeCVCount{}. The classifier
transfers across backdoor generators and is nearly as strong on \emph{inert} backdoors, ones
that never produce a wrong answer, as on working ones. Neither is
a property of a backdoor.

We contribute (i)~a construction for stored-pair corpora, in which one model writes both
...
authorship and most of the remainder to the attack prompt's framing; a one-sided variant,
regenerating only the honest class, substitutes one confound for another. We identify
(ii)~a second selection artifact the filter installs by defining the negative class through
attack failure, and (iii)~the construction and reporting practice these imply.
Our claims concern monitor training, selection and benchmarking on stored pairs, and our
measurement is of one APPS-derived family plus five agentic settings
(appendix~\ref{app:agentic}). ControlArena, the evaluation framework most of these corpora are
exported from, generates both classes live and is not implicated.
...
\includegraphics[width=0.85\linewidth]{f4-constructions.pdf}
\caption{\textbf{How a stored pair is built, and how we rebuild it.} Each row is one property of
the pair, marked \emph{confounded} where the two classes differ in it and \emph{controlled}
where one model or one prompt holds it constant. The backdoor row is the target: it is the only
difference a monitor should read, and the only one left once both controls are applied. Under
each construction is the bag-of-words classifier's AUROC on that construction's own pairs, so
the three are a ladder and not a paired comparison. The stored-benchmark \baselineB{} is on
the full \baselineBn{}-problem artifact, not the filtered split, whose value is \tfidfKoran{};
each arm's numbers are in Table~\ref{tab:gates} (notation: Table~\ref{tab:notation}).}
\label{fig:constructions}
\end{figure}

We use seven public corpora, whose
per-class provenance is the object of study: two views of one APPS-backdoor artifact
(appendix~\ref{app:schema}) and five agentic settings (appendix~\ref{app:agentic}). In the
APPS family the honest class is human and
...
Our primary protocol is one of the seven, the filtered, comment-stripped split
...
\cite{jha2026decorrelation}. The split is a filtered view of a \baselineBn{}-problem artifact
whose full pool the same-generator arms use, so two stored-pair values recur:
\tfidfKoran{} on this split, $B = \baselineB{}$ on
the artifact (Figure~\ref{fig:constructions}). Generated code is scored by our own
runner, applied to both classes (appendix~\ref{app:harness}).

Against that we build a corpus in which one model writes both classes on the same APPS
problems; the construction is run twice, with Sonnet~5 and with Haiku~4.5 as the generator.
The \emph{independent} arm writes the attack class from the problem statement alone,
so the writer changes and nothing else (the \emph{headline} rows of
Table~\ref{tab:gates}). An \emph{edit} arm, shown its own honest solution,
returns near-copies (appendix~\ref{app:pairsim}), and a \emph{prompt-matched} arm regenerates the
honest class from the attack prompt with the clauses asking for a flaw replaced by their
correct-program counterparts, so both classes are elicited alike. We sample one attack per
problem, fixed before its outcome is known, and define the attack class as the benchmark does,
by intent together with passing the public tests. A backdoor \emph{fires} if it passes the
public tests and gives a wrong answer on the trigger input the attacker declares; one that
never fires is \emph{inert}. Table~\ref{tab:notation} fixes the notation,
and the bands were registered before any arm ran: $\rho \le \rhoCollapse{}$ reads as authorship
(the \emph{collapse} band), $\rho > \rhoContent{}$ as content, the interval between as
neither (\emph{partial}).

\begin{table}[h]\centering\footnotesize
\caption{\textbf{Notation.} One classifier throughout; the arms differ only in the corpus
it reads.}
\label{tab:notation}
\begin{tabular}{@{}l l@{}}
\toprule
Symbol & Quantity \\
\midrule
$B$ & on the stored pairs, over all $n = \baselineBn{}$ problems \\
$B'$ & on the stored pairs, refitted on the problems one arm realises \\
$S$ & on a same-generator arm, where one model wrote both classes \\
$\rho = (S - 0.5)/(B' - 0.5)$ & the share of above-chance separability that survives \\
\bottomrule
\end{tabular}
\end{table}
...
Comment stripping succeeds on comment count and code length, driving both to chance
(Table~\ref{tab:protocol-baseline}, appendix). Two baselines on that same split do not move;
we call them \emph{floors}: a monitor at or below a floor has shown nothing beyond what the
corpus leaks.
Bag-of-words reaches \tfidfKoran{} and the no-code floor reaches
...
paired-bootstrap counts in appendix~\ref{app:protocol}). The two are not trained alike -- the monitors are
zero-shot, our classifiers fitted on the split's own train problems -- so the comparison states
what the corpus makes recoverable, not that a bag of words is the better monitor.

Three further measurements show that the bag-of-words classifier is not reading the backdoor
(the first two are pairwise accuracies, not AUROCs; appendix~\ref{app:protocol}).
It transfers across backdoor generators at \transfer{}: trained on one attacker's backdoors it identifies
another's, so the feature it has learned is one the attackers share. It is nearly as strong on
inert backdoors as on working ones, \pairInert{} against \pairWorking{}, although a backdoor
that never fires should be the harder case for a detector of sabotage. The feature it does read
is stylistic: \texttt{\_\_main\_\_} appears in 0.116 of attack samples against 0.000 of honest
ones, a gap the comment stripper leaves untouched and one that replicates on the train split,
in 152 of \splitTrain{} attack samples against 1 of \splitTrain{} honest ones. All three measurements are comment-stripped on both classes; the
unstripped values, inflated by the comment channel, are in appendix~\ref{app:stripped},
and a token-level version of the last test does not replicate (appendix~\ref{app:token}).
...
Holding the writer constant removes between a third and nine-tenths of the above-chance
separability (Table~\ref{tab:gates}, whose $B'$ is refitted on each row's own problems and
includes inert attacks): the two generators return $\rho = \sameGenRho{}$
(Sonnet 5) and \haikuRho{} (Haiku 4.5).

The pre-registered bands return a different verdict for each generator. Sonnet 5's interval
excludes the collapse band and spans the content boundary, leaving it inconclusive between partial
and content; Haiku 4.5's lies inside the collapse band. Sample size does not account for the fall
(appendix~\ref{app:robust}). Two further rows are \emph{strict}, keeping only attacks that
fire; a single sampled attack does so \headlineFireShare{} of the time, so these rows rest on
\strictN{} pairs
and agree with the headline rows at low power (appendix~\ref{app:secondpass}).
...
The two generators respond differently to the attack prompt. On Sonnet 5 what survives is lexical rather
than structural: length alone falls to \sameGenLen{} while TF-IDF holds, and of the
feature-ablation masks we
apply only identifier normalisation moves $S$ (appendix~\ref{app:robust}). The top
...
attack prompt elicits. Haiku 4.5 never adopts that layout in any of its three classes --
honest, attack, or prompt-matched honest
(appendix~\ref{app:secondpass}) -- which is why the two generators part. Holding the prompt
constant as well as the model moves them back toward each other, to
$\rho = \matchedRho{}$ on Sonnet 5 (Table~\ref{tab:gates}) and $\rho = \haikuMatchedRho{}$ on
Haiku 4.5 (appendix~\ref{app:secondpass}). On Sonnet this removes most of what
is left. The original and the prompt-matched honest classes -- both correct, no attack on
either side -- already separate at
\promptOnly{}, so the prompt carries framing and no backdoor at all; on Haiku 4.5 that same
floor contains chance.

Regenerating only one class fails: Qwen3-Coder-30B writing the honest
class against the stored backdoors separates \emph{better} than the stored pairs do,
$\rho = \armOneRho{}$, one authorship gap replaced by another
(appendix~\ref{app:robust}).

The same inflation appears with an LLM monitor as the detector. We put a zero-shot monitor
prompt to the model that wrote both classes, the most
favourable case, since it reads its own output. It scores \monStoredHead{} on the
stored pairs and \monitorS{} on the same-generator pairs, a monitor-$\rho$ of \monRhoHead{},
below the classifier's \sameGenRho{}; on the strict pairs the ratio is \monRhoStrict{}, and a
monitor that wrote neither class returns nearly the same two ratios
(appendix~\ref{app:monitors}).

The split selects on a second property besides authorship. Labelling by
\texttt{backdoor\_works} defines the negative class by attack failure rather than by
honesty: \inertShare{} of the test split's labelled backdoors do not work, and a
problem whose backdoor fails contributes its human honest solution instead -- the label is
set before any code is read. Attack problems also carry half as many test
cases (\testCasesAttack{} against
\testCasesHonest{}), so problem difficulty correlates with the label, which the
no-code floor's \nocodeCV{} reads.
...
A high floor is evidence about a corpus, not about results computed on it. Ours show the
label is recoverable without reading a backdoor, not that any monitor
recovers it that way, and no number here corrects a published monitor score.

The control arm is limited by the generator we had to substitute. The stored backdoors'
writer, \texttt{claude-3-7-sonnet-thinking}, is retired, so
...
monitor, bounds the content share of the separability from above instead of partitioning it.
The arm is not bit-reproducible: it ran through a metered API that does not expose sampling
...
published. Two interpreter-era artifacts, human solutions written for an older Python,
fall on opposite classes, so $B$ carries a small
execution-environment channel the same-generator arm does not
...
prompt framing, and one harness should verify both. Matched construction is precedented in
code-detection benchmarks
\cite{guo2025codemirage} but has not reached backdoor detection.

The definition of the negative class needs the same treatment. Defining it through attack
failure ties the label to the attacker rather than to the code; dropping the failed problems
removes that, as does sampling the class independently of attack success. A monitor number is
interpretable only against the floors of its own split, so a no-code and a
bag-of-words floor belong beside it, each against a permutation null, with the split and
preprocessing released alongside (ours: MIT, appendix~\ref{app:release}).
...
zero-shot prompted while the bag-of-words classifier is trained on the benchmark's own
...
\label{app:protocol}
The no-code floor is cross-validated within the test split because the released
...
\textbf{edit} (shown its own solution) & \simEdit{} & \simEditHigh{} & \simEditLow{} & authorship and similarity \\
...
$\rho = \editRho{}$. In the one-sided arm of \S\ref{sec:results}, where only the honest half
was regenerated, the classifier reaches \armOneG{} against
...
\label{app:agentic}
Removing the authorship gap does not remove every floor. In five settings where the same three
models write both classes, so no authorship confound is possible, the surviving floor is
structural: \bashFloor{} (\texttt{bash}, on its null) to \agentdojoFloor{}
(\texttt{agentdojo}) from character count alone (Figure~\ref{fig:corpora}).
...
code, whereas both of our classes are competitive-programming code, so an identifier

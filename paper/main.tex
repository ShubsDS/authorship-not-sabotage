...
\newcommand{\sameGenRhoLo}{0.627}
\newcommand{\sameGenRhoHi}{0.742}
...
\newcommand{\strictRhoLo}{0.700}
\newcommand{\strictRhoHi}{1.043}
...
returns \emph{inconclusive between partial and content}. Authorship accounts for roughly a third of the separability and surface content
of the attack class for the rest, and what survives is lexical: length-only falls to
\sameGenLen{} while TF-IDF holds. The pairs are independent programs (similarity
...
holds \sameGenS{}: what survives there with authorship held constant is a lexical trace of
the attack instruction, not a backdoor a monitor recognises. Our single-shot attacks fire given a pass at \fireGivenPassOurs{} against the stored
attacker's \fireGivenPassStored{}, which executes and iterates: the stored attack class is
a survivor set. The interval rule was changed before the retry pool was read, and we say
so: the $\pm2$ fold-sd interval first coded is far wider than a 95\% interval over
problems, so $S$ routes on a bootstrap over problems.

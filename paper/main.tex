% =====================================================================================
%  Authorship Confounds in AI-Control Monitor Evaluation: Partial-Input and Bag-of-Words Floors, and a Same-Generator Corpus
...
% Four lines, broken by hand at each phrase boundary; unbroken, LaTeX strands
% "Evaluation:" alone on a line. Wording checked against the body, not chosen by ear:
%   "Monitor Evaluation" not "Monitoring" - the confound is in how monitors are
%     evaluated, which the first Limitations paragraph is explicit about.
%   "Partial-Input and Bag-of-Words Floors" not "Baseline Floors" - this is the
%     abstract's closing phrase and Recommendation 2 verbatim, and it says what the
%     two floors READ, which is the point. "Baseline floor" is redundant (a baseline
%     of this kind IS the floor) and names neither. Not "content-blind" either: that
%     is true of the no-code floor only, and would mislabel the headline 0.806.
\title{Authorship Confounds in\\AI-Control Monitor Evaluation:\\Partial-Input and Bag-of-Words Floors,\\and a Same-Generator Corpus}
...
% =====================================================================================
\begin{abstract}
Trusted monitors are tuned and ranked on stored pairs of code, pairing for one programming
problem an honest solution with a solution carrying a planted backdoor. The two classes are built
...
backdoors as on working ones, so no property of a backdoor explains it. Regenerating only the
honest half raises separability further. One model writing both classes removes between
...
We use seven public corpora, whose per-class provenance is the object of study. In the APPS
family the honest class is human and the attack class is one of ten named models.
...
Our headline protocol is the filtered, comment-stripped split \cite{koran2026ensemble}:
\splitTrain{} train problems (\splitTrainItems{} paired items) and \splitTest{} test items
(\splitAttack{} attack, \splitHonest{} honest), labelled by \texttt{backdoor\_works}, reconstructed with the released harness code
...
(Table~\ref{tab:protocol-baseline}, appendix). Two floors on that same split do not move.
Bag-of-words reaches \tfidfKoran{} (95\% CI \tfidfKoranLo{}--\tfidfKoranHi{} by bootstrap
...
of them with a paired-bootstrap interval excluding zero. The no-code classifier reaches
...
reached the carrier it was aimed at and left the two beside it standing.
...
attacker's backdoors it identifies another's, so what it has learned belongs to no single
model's sabotage style. It is also nearly as strong on \textbf{inert} backdoors
...
\texttt{\_\_main\_\_} appears in 0.116 of attack samples against \textbf{0.000} of
...
separability} (Table~\ref{tab:gates}). We report the \textbf{independent} arm, in which the
...
What separates the two generators is the attack prompt. On Sonnet 5 the surviving signal is
...
model that wrote both classes. In this field's vocabulary it is an untrusted monitor rather
than a trusted one, which makes it the most favourable case available, since it is reading its
own output. On the \emph{stored} pairs over the headline problems it scores
...
Regenerating only the honest half makes separation worse. With an open-weight model writing
the honest class against the stored backdoors, $\rho = \armOneRho{}$
...
to hide, so difficulty and test count correlate with the label. That correlation is where the
content-blind \nocodeCV{} comes from.
...
\textbf{A high floor is evidence about a corpus; it does not settle results computed on
that corpus.} Our floors show that the label is recoverable without reading a backdoor. They
leave open whether any monitor recovers it that way, and no number here corrects a published
monitor score.

\textbf{The generator, and what $\rho$ can therefore be read as.}
...
parameters, with thinking off although the stored attacker was a thinking model. The two generators are
...
same way is untested. Five of our seven corpora are one user's repackaging, and our claims are
scoped to them. Two interpreter-era artifacts reach $B$ but not the same-generator arm

...
appendix~\ref{app:pairsim}. We take one draw per problem, fixed before its outcome is known, and
...
$\curveHeadline{} \pm \curveHeadlineSd{}$. Two further rows are \emph{strict}, keeping only
attacks that pass the tests \emph{and} fire on their trigger, selected by the benchmark's own
retry-and-keep. Only \headlineFireShare{} of Sonnet 5's draws fire, which leaves \strictN{}
pairs, so we read those rows through $\rho$ alone and they agree with the headline rows at low
power; \haikuFireShare{} of Haiku 4.5's fire (\haikuFireInRow{} of \haikuN{}), too few for a
strict row at all.

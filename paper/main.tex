...
\newcommand{\sameGenPairwise}{0.790}
\newcommand{\sameGenLen}{0.5851}       % length-only carrier: to chance
\newcommand{\sameGenSurface}{0.6087}
\newcommand{\sameGenSim}{0.659}        % pair similarity, same problems
\newcommand{\sameGenSimStored}{0.606}  % stored pairs on the same problems
...
\newcommand{\survivorLine}{0.70}       % pre-stated contamination line
...
\newcommand{\retryProblems}{1{,}112}
\newcommand{\retryMaxDraws}{13}
...
\newcommand{\monitorStrictPairwise}{0.816}
...
\newcommand{\defMainHonest}{39\%}        % honest side
...
solutions against the stored backdoors, separability \emph{rises}. Holding authorship constant
...
whole story and attributes the remainder to surface content of the attack class. We
recommend that
...
separability to authorship and the rest to surface content of the attack class, with a
negative result on the way: regenerating one half of a stored pair swaps the confound
rather than removing it; (iii)~a
...
open-weight monitors have published per-item scores. Five populations (the raw artifact, this split, pooled, matched pairs, within-task) and
two metrics appear below; every comparison names its split, metric and preprocessing. Generated code is scored by our own runner, which
...
each protocol. The fix works: code length is driven to
...
backdoor. A fourth, token-level test \cite{orel2026aicd} does not replicate here
(separation \tokenSep{}, permutation $p = \tokenP{}$; appendix~\ref{app:token}), a
register effect; and one authorship leak, author bylines inside string literals, survives
the field's own comment stripper (appendix~\ref{app:stripped}).
...
problems), separation \emph{rises} to $G = \armOneG{}$, $\rho = \armOneRho{}$; TF-IDF gains \armOneTfidfDelta{} while the
length carrier collapses to \armOneLenG{}. Substituting the honest author replaced one authorship confound with a larger one.

\paragraph{Holding authorship constant needs care about what else changes.}
We generate the attack class twice from one honest class: an \textbf{edit} arm shown its
own honest solution, whose pairs are near-copies, and an \textbf{independent} arm shown
only the problem statement, whose pairs match the stored regime of independent programs
(Table~\ref{tab:similarity}, appendix). Only the second changes authorship alone.
...
attack class defined, as in the benchmark, by intent plus passing the public tests, and
$B'$ inert-inclusive over the same problems --- $S = \sameGenS{}$ (95\% CI
\sameGenLo{}--\sameGenHi{}; $n = \sameGenN{}$ problem-disjoint pairs) against
$B' = \sameGenBprime{}$, so $\rho = \sameGenRho{}$ (\sameGenRhoLo{}--\sameGenRhoHi{}). The
interval excludes the collapse band and spans the content line, so the pre-registered rule
returns \emph{inconclusive between partial and content}. Authorship accounts for roughly a third of the separability, and what survives is lexical: length-only falls to
\sameGenLen{} while TF-IDF holds. It is not the trigger: masking numerals and comparison
operators leaves $S$ unchanged, while normalising identifiers takes it to \sameGenSid{}
($\rho = \sameGenRhoId{}$) with $B'$ untouched, and the top attack-side features are
...
$\rho$ is an upper bound on content. The pairs are independent programs (similarity
\sameGenSim{}; stored pairs on the same problems \sameGenSimStored{}), and at
...
\emph{and} fire, kept by the benchmark's own retry-and-keep procedure, the filtering this
paper criticises, inherited deliberately --- agrees: $\rho = \strictRho{}$
...
survivors and non-survivors separate at only \survivorAuroc{}, and $S$ with non-firing
attempts on the same problems is \survivorSwapS{} against \survivorSwapSref{}. The edit arm bounds $S$ under near-copies
(similarity \editSim{}): \editS{}, $\rho = \editRho{}$. A trusted-monitor prompt to the
same model --- the one that wrote both classes --- scores \monitorStrict{}
...
attacker's \fireGivenPassStored{}, which executes and iterates: the stored attack class is
a survivor set. Intervals are bootstraps over problems, resampling $S$ and $B'$ jointly for
$\rho$; this replaced a $\pm2$ fold-sd rule before the retry pool was read.
...
(\splitAttack{} attack / \splitHonest{} honest), comments stripped. Monitor scores from
\cite{jha2026decorrelation} (MIT); median \monMedian{}, max \monMax{}. \textbf{Two caveats.}
...
(Figure~\ref{fig:corpora}, appendix). \texttt{bash} sits on its
...
\item \textbf{State the split, the metric and the preprocessing} whenever monitors are
      compared, and \textbf{release the split and the scripts}. Ours accompany this paper as supplementary material under MIT: the
      same-generator split (\relHonest{} honest, \relEdit{} edit-arm and \relIndep{}
      independent-arm programs from one model, plus all \relRetryDraws{} retry-pool draws),
      pair lists under both selection rules --- outcome-blind (\relPassFirst{} passing,
      \relFireFirst{} firing independent-arm pairs) and the benchmark's own survivor
      selection (\relPassBest{} and \relFireBest{}) --- a dataset card and a manifest
...
\citet{poliak2018hypothesis}, not that any monitor exploits that.
...
metered commercial API. That API does not expose its sampling parameters, so the arm is not bit-reproducible as the
open-weight arms are, and thinking was off although the stored attacker was a thinking
model. We do not
length-match: in deployment the untrusted model chooses its trajectory length.
...
repackaging; claims are scoped to those releases. Two interpreter-era artifacts
fall on opposite classes (a \texttt{stdin} without \texttt{.buffer}, fixed; \texttt{fractions.gcd}, removed in
Python 3.9, not fixable); the same-generator arm is immune, $B$ is not.
...
the trigger as firing), a dataset card, and a manifest of row counts and checksums, under
the MIT licence.

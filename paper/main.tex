...
The fix works, on the two carriers \S1 names
(Table~\ref{tab:protocol-baseline}, appendix). What it does not reach, on that same split, is a
...
Three measurements point the same way; every number here is comment-stripped on
both classes, the unstripped values, inflated by the comment channel, being in
appendix~\ref{app:stripped}. First, the classifier \emph{transfers across
...
ones (\pairWorking{}); if it were detecting sabotage, backdoors that do not function should
be much harder. Third, the residual carrier is stylistic rather
...
backdoor, and a token-level test \cite{orel2026aicd} does not replicate here
...
honest class against the same backdoors, separation \emph{rises}, to $\rho = \armOneRho{}$
(appendix~\ref{app:robust}): substituting the honest author replaced one authorship confound
with a larger one.
...
\textbf{One model writing both classes removes between a third and nine-tenths of the
separability.} We report the \textbf{independent} arm --- the attack class written from the
problem statement alone, which changes authorship and nothing else, an \textbf{edit} arm
shown its own honest solution returning near-copies (appendix~\ref{app:pairsim}). One
outcome-blind draw per problem, the attack class as the benchmark defines it --- by intent plus passing the public tests (\headlineFireShare{} of these draws
fire, the same-generator analogue of the split's own \inertShare{} inert class) --- and $B'$
inert-inclusive over the same problems (Table~\ref{tab:gates}). The first generator's
interval excludes the collapse band and spans the content line, so the pre-registered rule
returns \emph{inconclusive between partial and content}; the second's lies inside the
collapse band. At $n = \sameGenN{}$, $B$ under the identical pipeline is $\curveHeadline{}
\pm \curveHeadlineSd{}$, so the drop is not sample size.

\begin{table}[h]\centering\footnotesize
\setlength{\tabcolsep}{4pt}
\caption{\textbf{Gate S: what is left when one model writes both classes.} $S$ is the
...
appendix~\ref{app:robust}. \headlineInert{} of the first generator's headline attacks never
fire and \haikuInert{} of the second's; the strict rows are all-firing.}
\label{tab:gates}
...
\textbf{Holding the attack prompt constant as well removes most of what is left.} What
survives on the first generator is lexical: length-only falls to \sameGenLen{} while TF-IDF
holds. It is not the trigger --- masking numerals and comparison operators leaves $S$
unchanged, while normalising identifiers takes it to \sameGenSid{} ($\rho =
\sameGenRhoId{}$) --- and the top attack-side features are \texttt{def main}
(\defMainAttack{} vs \defMainHonest{} of sides) and \texttt{data[}: the layout the attack
prompt elicits. Regenerating the honest class under that framing minus the flaw sentence
takes $\rho$ to \matchedRho{}, and the two honest classes alone, no attack on either side,
separate at \promptOnly{}: what the prompt removes is framing, not backdoor. On the second
generator neither moves --- \haikuSMatched{} with the prompt matched ($\rho =
\haikuMatchedRho{}$) over a prompt-only floor of \haikuPromptFloor{} whose interval contains
chance: a generator that never adopts the prompt's layout. The strict rows --- attacks that pass \emph{and} fire under the
benchmark's own retry-and-keep, the rows that follow our Recommendation 3 --- agree at low
power, read through $\rho$ alone because $B$ at $n = \strictN{}$ is $\curveStrict{} \pm
\curveStrictSd{}$.

\textbf{Monitors are inflated the same way.} A zero-shot monitor prompt, answered by the
model that wrote both classes, scores \monStoredHead{} on the \emph{stored} pairs over those
problems and \monitorS{} (\monitorSLo{}--\monitorSHi{}) on the headline pairs, a
...
same two ratios (\monHaikuRhoHead{}, a bound at half missingness, and \monHaikuRhoStrict{}).
Classifier and monitor are independent pair by pair ($\phi = \monitorClfPhi{}$). Our
single-shot attacks fire given a pass at \fireGivenPassOurs{} against the stored attacker's
\fireGivenPassStored{}: the stored attack class is a survivor set.
...
Attack problems also carry roughly half as many test cases
(\testCasesAttack{} versus \testCasesHonest{}) --- fewer tests, easier to backdoor
undetectably --- so difficulty and test count correlate with the label with no code read at
all, which is where the content-blind \nocodeCV{} comes from.

\paragraph{The negative control comes free.}
In five same-generator agentic settings, where the same three models write both classes and
no authorship confound is possible, the surviving floor is structural: \bashFloor{}
(\texttt{bash}, on its null) to \agentdojoFloor{} (\texttt{agentdojo}) on character count
alone (Figure~\ref{fig:corpora}, appendix).
...
\item \textbf{State the split, the metric and the preprocessing} whenever monitors are
      compared, and \textbf{release the split and the scripts}. Ours accompany this paper
      under MIT (appendix~\ref{app:release}).
\end{enumerate}

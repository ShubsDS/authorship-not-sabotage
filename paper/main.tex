...
% =====================================================================================
\begin{abstract}
Trusted monitors are tuned and ranked on stored pairs of code: for one programming problem, an
honest solution and a solution carrying a planted backdoor. The two classes are built
...
ranked on it, and a classifier reading no code at all, only problem metadata, scores above most
of them. The signal transfers across backdoor generators and is nearly as strong on inert
backdoors as on working ones, so it is not a property of a backdoor. Regenerating only the
honest half makes it worse: separability rises. One model writing both classes removes between
a third and nine-tenths of it, on two generators that differ because only one adopts the attack
prompt's layout; matching that prompt removes most of the rest. A zero-shot monitor is inflated
the same way, scoring \monStoredStrictAbs{} on stored pairs and \monitorStrict{} on firing
backdoors from the same author. Stored-pair evaluations should report partial-input and
...
untrusted model's output and flags sabotage \cite{greenblatt2023aicontrol}. Monitors are
developed, tuned and ranked almost entirely on \emph{stored} honest/attack pairs: fixed
...
jha2026decorrelation}. The two classes come from different authors. Honest solutions are
...
That these pairs leak is known, and the corpora are preprocessed against it. Koran
et al.\ keep only working honest solutions and working backdoors and strip every comment
\cite{koran2026ensemble}; Control Tax reports the pass-tests pre-filter and appendices
``Monitoring failed backdoors'' \cite{terekhov2025controltax}; the comment channel has a paper
of its own \cite{sreevatsa2026legibility}. Jha's harness ships a \texttt{--leak-check} flag
with two trivial baselines in it: a \texttt{\#}-count detector falls from \hashRaw{} to
\hashStripped{} once comments go, and code length alone sits at \lenFiltered{}
\cite{jha2026decorrelation}. Partial-input baselines are the standard instrument
\cite{gururangan2018annotation, poliak2018hypothesis}, their failure does not certify a
dataset \cite{feng2019misleading}, and leakage is handled as a checklist rather than a verdict
\cite{kapoor2022leakage}. That bag-of-words separates human-written from model-written code is
established out-of-field \cite{orel2026aicd}, and detectors built on it match their classes:
CodeMirage gates on line and character count \cite{guo2025codemirage}, MultiAIGCD pairs to
shared problem definitions \cite{demirok2025multiaigcd}. Adjacent control work attacks other
axes: one subverts monitors with injected text \cite{terekhov2026adaptive}, another shows
attacker-side selection inflates safety estimates \cite{gewang2026attackselection}, and a
third removes transcript length as a nuisance covariate \cite{pinto2026calibration}.
...
generators and is nearly as strong on \textbf{inert} backdoors as on working ones. Neither is
...
\cite{jha2026decorrelation}, median \monMedian{}, max \monMax{}. \textbf{Two caveats.} (i)~The monitors are
zero-shot prompted while the bag-of-words classifier is \emph{trained} on the benchmark's own
paired train split: a statement about what the corpus makes recoverable, not a claim that
bag-of-words is a better monitor.
...
backdoor, and a token-level version of the test does not replicate here
...
\paragraph{Where authorship is already matched, a floor remains.}
...
Our floors show that the label is recoverable without reading a backdoor. They do not show
that any monitor recovers it that way, and no number here is a correction to a published
monitor score.
...
For any evaluation built on stored pairs:
\begin{enumerate}\itemsep1pt
\item \textbf{Generate both classes with the same model and the same prompt framing.} If the attack class is
      model-written, the honest class must be too: regenerating only one side
      substitutes one authorship gap for another.

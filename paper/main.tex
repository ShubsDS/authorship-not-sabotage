...
\textbf{Our generator is not the stored attacker, so $\rho$ bounds rather than partitions.}
The model that wrote the stored backdoors, \texttt{claude-3-7-sonnet-thinking}, is retired, so
both same-generator classes come from a later model of the same lineage. It differs from the
stored attacker in backdoor style, not only in authorship, so every $\rho$ here, classifier or
monitor, bounds how much of the separability is content rather than partitioning it. The arm
ran through a metered API that does not expose sampling parameters, so it is not
bit-reproducible, and thinking was off although the stored attacker was a thinking model. The
two generators are also compared on the \texttt{passes} eligibility alone: Haiku 4.5's strict

...
\monStoredHeadAbs{} on stored pairs to \monitorS{} on same-generator pairs. Stored-pair evaluations
should report a partial-input and a bag-of-words floor against permutation nulls, generate both
...
``Monitoring failed backdoors'' \cite{terekhov2025controltax};  one harness ships a \texttt{--leak-check} flag with
...
\cite{jha2026decorrelation}. Out of field, bag-of-words is known to separate human-written from
...
measurement is of one APPS-derived family plus five agentic settings. ControlArena's live evaluations generate both classes and are not implicated.
...
the three are a ladder and not a paired comparison; each arm's $B'$ and $\rho$ are in
Table~\ref{tab:gates}.}
...
problems. The \emph{independent} arm writes the attack class from the problem statement alone,
so the writer changes and nothing else; an \emph{edit} arm, shown its own honest solution,
returns near-copies (appendix~\ref{app:pairsim}); a \emph{prompt-matched} arm regenerates the
...
by intent together with passing the public tests. Table~\ref{tab:notation} fixes the notation,
and the bands were registered before any arm ran: $\rho \le \rhoCollapse{}$ reads as authorship,
$\rho > \rhoContent{}$ as content, the interval between them as neither.
...
It transfers across backdoor generators at \transfer{} pairwise: trained on one attacker's backdoors it identifies
...
at 152 against 1 of \splitTrain{}. All three measurements are comment-stripped on both classes; the
...
Holding the writer constant removes between a third and nine-tenths of the separability
(Table~\ref{tab:gates}); $B'$ is refitted on each arm's own problems and includes inert
attacks. With $\rho = 1$ meaning that holding the writer constant changed nothing, and
$\rho = 0$ that nothing survived, the arms return \sameGenRho{} and \haikuRho{}.

The pre-registered rule returns a different verdict for each generator. Sonnet 5's interval
excludes the collapse band and spans the content line, leaving it inconclusive between partial
...
pass the public tests \emph{and} give a wrong answer on the trigger input the model declares;
single samples do so \headlineFireShare{} of the time, so these rows rest on \strictN{} pairs
...
The attack prompt separates the two generators. On Sonnet 5 what survives is lexical rather
than structural: length alone falls to \sameGenLen{} while TF-IDF holds, and of the masks we
...
Regenerating one class rather than both does not work: an open-weight model writing the honest
class against the stored backdoors separates \emph{better} than the stored pairs do,
$\rho = \armOneRho{}$, having replaced one authorship gap with another
(appendix~\ref{app:robust}).

The same inflation appears when the detector is an LLM monitor rather than a bag-of-words
classifier. We put a zero-shot monitor prompt to the model that wrote both classes, the most
favourable case available since it is reading its own output. It scores \monStoredHead{} on the
...
monitor that wrote neither class returns the same two (appendix~\ref{app:monitors}).

The split selects on a second property besides authorship. Labelling by
\texttt{backdoor\_works} defines the negative class by attack failure where the criterion
should be honesty: \inertShare{} of the test split's labelled backdoors do not work, and a
problem whose backdoor fails contributes its human honest solution instead, before any code is
read. Attack problems also carry half as many test cases (\testCasesAttack{} against
\testCasesHonest{}), so problem difficulty correlates with the label, which is what the
content-blind \nocodeCV{} reads.
...
A high floor is evidence about a corpus, not about results computed on it. Ours show that the
label is recoverable without reading a backdoor, but say nothing about whether any monitor
...
removes that, as does sampling the class independently of attack success. A monitor number is in
turn interpretable only against the floors of its own split, so a partial-input and a
bag-of-words floor belong beside it, each against a permutation null, with the split and its
preprocessing released alongside (ours are MIT, appendix~\ref{app:release}).

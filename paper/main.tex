...
chance to 1.0. Each is measured on the problems its own construction realises (\baselineBn{},
\sameGenN{} and \matchedN{} pairs), so the three are a ladder and not a paired comparison, and
none of them is the \tfidfKoran{} floor of \S\ref{sec:results}, which is measured on the split the
monitors are ranked on. Every arm's refitted baseline $B'$ and its $\rho$ are in
Table~\ref{tab:gates}.}
...
at 152 against 1 of \splitTrain{}. A token-level version of that last test does not replicate
(appendix~\ref{app:token}). All three measurements are comment-stripped on both classes; the
...
(Table~\ref{tab:gates}). We report the \emph{independent} arm, in which the model writes the
attack class from the problem statement alone, so the writer changes and nothing else; an
\emph{edit} arm, shown its own honest solution, returns near-copies and is reported in
appendix~\ref{app:pairsim}. We sample one attack per problem, fixed before its outcome is known, and
define the attack class as the benchmark does, by intent together with passing the public tests.
$B'$ is refitted on the same problems and includes inert attacks.
...
and content; Haiku 4.5's lies inside the collapse band. Sample size does not account for the
fall: at $n = \sameGenN{}$ the stored-pair baseline under the identical pipeline is
$\curveHeadline{} \pm \curveHeadlineSd{}$. Two further rows are \emph{strict}: they keep only
attacks that pass the public tests \emph{and} give a wrong answer on the trigger input the model
declares. Single samples rarely do both, \headlineFireShare{} of Sonnet 5's and
\haikuFireShare{} of Haiku 4.5's (\haikuFireInRow{} of \haikuN{}), so for Sonnet 5 the strict
rows follow the benchmark's own practice of sampling each problem repeatedly and keeping the
survivors, which yields \strictN{} pairs. We read those rows through $\rho$ alone and they agree
with the headline rows at low power. Haiku 4.5 has too few for a strict row at all
(appendix~\ref{app:secondpass}).
...
makes it the most favourable case available, since it is reading its own output. On the stored pairs over the headline problems it scores
...
\monHaikuRhoStrict{}. Among attacks that pass the tests, ours give a wrong answer on their trigger at
\fireGivenPassOurs{} against the stored attacker's \fireGivenPassStored{}, which makes the
stored attack class a survivor set.

The split selects on a second property besides authorship. Labelling by
\texttt{backdoor\_works} defines the negative class at the problem level by attack

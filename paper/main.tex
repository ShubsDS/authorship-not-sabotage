...
open-weight monitors have published per-item scores. Every comparison below names its split, metric and preprocessing. Generated code is scored by our own runner, which
...
and the edit arm bounds $S$ under near-copies (appendix~\ref{app:robust}). A trusted-monitor prompt to the
...
the test split by $5\times5$ cross-validation (appendix~\ref{app:protocol}); on a single
held-out slice of the same items it is \nocodeHeldout{}
...
null) to \agentdojoFloor{} (\texttt{agentdojo}) from character count alone. A
...
\item \textbf{State the split, the metric and the preprocessing} whenever monitors are
      compared, and \textbf{release the split and the scripts}. Ours accompany this paper as
      supplementary material under MIT: the same-generator split, both selection rules as
      pair lists, every retry-pool draw, the scripts that produced the numbers, a dataset
      card and a manifest (appendix~\ref{app:release}).
\end{enumerate}
...
\armOneTfidfDelta{} while the length carrier collapses to \armOneLenG{}.

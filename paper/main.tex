...
of the separability, on two generators that differ because only one adopts the attack prompt's
layout. Matching that prompt on the honest side removes most of what remains. What survives
on attacks that actually fire is consistent with surface content a monitor also reads. A zero-shot monitor is inflated the same way, scoring \monStoredStrictAbs{} on stored
...
appendices ``Monitoring failed backdoors'' \cite{terekhov2025controltax}; an entire paper is
given to the comment channel \cite{sreevatsa2026legibility}. The sharpest treatment
...
published standard. Adjacent control work attacks other axes: one subverts monitors with
injected text \cite{terekhov2026adaptive}, another shows attacker-side selection inflates
safety estimates \cite{gewang2026attackselection}, and a third removes transcript length as a
nuisance covariate \cite{pinto2026calibration}; we publish one as a per-benchmark floor.
...
Comment stripping works on the two carriers \S1 names, driving both to chance

...
comment-stripped split, a \emph{trained} bag-of-words classifier scores above roughly
four-fifths of the open-weight monitors ranked on it. A classifier that reads no code at
all --- only problem metadata --- scores above most of them. We trace the residue to authorship rather
...
construction --- one model writing both classes --- removes between a third and nine-tenths
of the separability. The share of above-chance signal that survives is $\rho = \sameGenRho{}$
for Claude Sonnet 5 and $\rho = \haikuRho{}$ for Claude Haiku 4.5. The two differ because
only Sonnet adopts the attack prompt's layout. Matching that prompt on the honest side removes
most of what remains. What survives on attacks that actually fire is consistent with surface
content a monitor also reads. A zero-shot monitor is inflated the same way, scoring \monStoredStrictAbs{} on stored
...
Three measurements point the same way. Every number here is comment-stripped
on both classes; the unstripped values, inflated by the comment channel, are in
...
$n = \baselineBn{}$ pairs. The routing rule and its bands were fixed before any arm ran.
Retained signal is $\rho = (G - 0.5)/(B - 0.5)$: $\rho = 1$ means holding authorship
constant changed nothing, $\rho = 0$ that nothing survived. We pre-registered
$\rho \le \rhoCollapse{}$ as authorship and $\rho > \rhoContent{}$ as content. With an open-weight model writing the
...
separability} (Table~\ref{tab:gates}). We report the \textbf{independent} arm: the attack
class is written from the problem statement alone, so authorship changes and nothing else.
An \textbf{edit} arm, shown its own honest solution, returns near-copies instead
...
\headlineFireShare{} of those draws fire; $B'$ is inert-inclusive over the same problems.
...
generator's lies inside the collapse band. The drop is not sample size: at $n = \sameGenN{}$,
$B$ under the identical pipeline is $\curveHeadline{} \pm \curveHeadlineSd{}$. Two rows are
\emph{strict}: they keep only attacks that pass the tests \emph{and} fire on their trigger,
selected by the benchmark's own retry-and-keep as our Recommendation 3 asks. Only
\strictN{} pairs meet it,
so we read them through $\rho$ alone, $B$ at that $n$ being $\curveStrict{} \pm
\curveStrictSd{}$; they agree with the headline rows at low power.
...
\textbf{The two generators differ because of the attack prompt, not because the measurement
is noisy.} What survives on Sonnet 5 is lexical: length alone falls to \sameGenLen{} while
TF-IDF holds. It is not the trigger. Masking numerals and comparison operators leaves $S$
unchanged; only normalising identifiers moves it, to \sameGenSid{} ($\rho = \sameGenRhoId{}$).
The top attack-side features are \texttt{def main} (\defMainAttack{} of attack programs
against \defMainHonest{} of honest ones) and \texttt{data[}: the program layout the attack prompt
elicits. Haiku 4.5 never adopts it. \texttt{def main} and \texttt{sys.stdin} appear in no
program of any of its three classes, which read input with \texttt{input(} throughout
(appendix~\ref{app:secondpass}). So when the prompt is held
constant as well as the model, the two move toward each other, to $\rho = \matchedRho{}$ and
$\rho = \haikuMatchedRho{}$. On Sonnet that removes most of what is left: the two honest
classes alone, no attack on either side, already separate at \promptOnly{}, so what the
prompt removes is framing, not backdoor. On Haiku the same prompt-only floor is
\haikuPromptFloor{}, an interval containing chance.

\textbf{A zero-shot monitor is inflated the same way.} We put that prompt to the model that
wrote both classes. On the \emph{stored} pairs over the headline problems it scores
\monStoredHead{}; on the same-generator pairs, \monitorS{} (\monitorSLo{}--\monitorSHi{}).
That is a monitor-$\rho$ of \monRhoHead{} (\monRhoHeadLo{}--\monRhoHeadHi{}), below the
classifier's \sameGenRho{}. On the strict pairs the two scores are \monStoredStrict{} and
\monitorStrict{} (\monitorStrictLo{}--\monitorStrictHi{}), a monitor-$\rho$ of
\monRhoStrict{} (\monRhoStrictLo{}--\monRhoStrictHi{}). A monitor that wrote neither class
...
against the stored attacker's \fireGivenPassStored{}: the stored attack class is a survivor
set.
...
undetectably --- so difficulty and test count correlate with the label, which is where the
content-blind \nocodeCV{} comes from.
...
\citet{feng2019misleading} warn that a partial-input baseline's \emph{failure} does not prove
a dataset clean; the converse binds us. Our floors show the label is recoverable without
reading a backdoor \citep{gururangan2018annotation, poliak2018hypothesis}, not that monitors
exploit it.
...
come from a later model of the same lineage, called through a metered API that does not
expose its sampling parameters, so the arm is not bit-reproducible; thinking was off
although the stored attacker was a thinking model. The stored attacker and ours also
differ in backdoor style, not only in authorship, so every $\rho$ here --- classifier or
monitor --- bounds how much of the separability is content rather than partitioning it. And
the two generators are compared on the \texttt{passes} eligibility alone: Haiku 4.5's strict
row was refused at \haikuUsable{} pairs against our 50-pair floor.

\textbf{Scope, provenance and the harness.} Whether stored-pair corpora leak this way is
unknown: five of our seven corpora are one user's repackaging, and our claims are scoped to
them. Two interpreter-era artifacts reach $B$ but not the same-generator arm
...
claim that a bag of words beats a monitor here. Classifier and monitor are independent pair
by pair on the same-generator pairs ($\phi = \monitorClfPhi{}$): they are not two readings of
one signal.
...
documents. We do not length-match either class; the untrusted model chooses its own

...
\paragraph{The confound in the schema.}
...
Six of the seven declare no licence \cite{koran2026highquality,
...
paired-bootstrap counts in appendix~\ref{app:protocol}). The preprocessing reached the
carrier it was aimed at and left the two beside it standing.
...
\paragraph{The same-generator arms.}
...
\paragraph{The monitor arm.}
The same inflation reaches an LLM monitor. We put a zero-shot monitor prompt to the
...
\paragraph{A second selection artifact.}
...
\paragraph{Agentic corpora.}
In five settings where the same three models write both classes, so no authorship confound is
possible, the surviving floor is structural: \bashFloor{} (\texttt{bash}, on its
...
\paragraph{The scope of a floor.}
...
\paragraph{Scope, provenance and the harness.}
We have not tested stored-pair corpora beyond
...
\item Generate both rollouts from one model under one prompt framing, and verify both with one
      harness. If the attack class is model-written, the honest class must be too; regenerating
      one side substitutes one authorship gap for another. Matched construction is precedented
      in code-detection benchmarks \cite{guo2025codemirage}, but we are not aware of it
      reaching backdoor detection.
\item Report a partial-input floor and a bag-of-words floor beside every monitor number, each
      against a permutation null.
\item Do not define the negative class by attack failure. Drop the problems whose attacks
      failed, or sample the negative class independently of attack success.
\item State the split, the metric and the preprocessing whenever monitors are compared, and
      release all three. Ours are MIT (appendix~\ref{app:release}).
\end{enumerate}

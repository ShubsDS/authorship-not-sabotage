...
The confound is visible in the record itself. The canonical stored-pair artifact for APPS
...
(Table~\ref{tab:schema}): provenance is tracked for one class because only one class has a
model behind it. Figure~\ref{fig:constructions} sets that construction beside the one we build
against it.
...
The corpora built on that artifact inherit the asymmetry. We use seven public corpora, whose
...
One of the seven sets the evaluation surface. Our headline protocol is the filtered,
comment-stripped split \cite{koran2026ensemble}: \splitTrain{} train problems (\splitTrainItems{} paired items) and \splitTest{} test items
...
Three properties of that bag-of-words classifier say what it is reading, and none of them is
the backdoor. Every number in this paragraph is comment-stripped on both classes; the unstripped
...
If authorship is what the floors read, then holding it constant should lower them, and
measuring by how much needs a scale. Under the identical pipeline the human-honest baseline is $B = \baselineB$ over
...
Regenerating one side only is the cheaper repair, and it does not work. With an open-weight
model writing the honest class against the stored backdoors, separation is worse than it was
before, at $\rho = \armOneRho{}$ (appendix~\ref{app:robust}): one authorship gap has been
substituted for another.

None of this depends on the instrument being a bag-of-words classifier: the same inflation
reaches an LLM monitor. We put a zero-shot monitor prompt to the model that wrote both
classes. In this field's vocabulary it is an untrusted monitor, which
...
Authorship is not the only thing this split selects on. Labelling by
...
Nor does closing the authorship gap leave a corpus with no floor at all. In five settings
where the same three models write both classes, so no authorship confound is possible, the
surviving floor is structural: \bashFloor{} (\texttt{bash}, on its
...
The control arm carries a limit of its own, in the generator that had to stand in. The model
that wrote the stored backdoors, \texttt{claude-3-7-sonnet-thinking}, is retired, so
...
Our scope is narrow in three further ways. We have not tested stored-pair corpora beyond these
for the same leak. Five of our seven corpora are one user's repackaging, and our claims are
...
The construction this implies is a single rule: one model should write both classes under one
...
The label needs the same care as the authorship. Defining the negative class through attack
failure ties it to the attacker rather than to the code, and puts a content-blind classifier

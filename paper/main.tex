...
way: regenerating one half of a stored pair swaps the confound rather than removing it. We add
(iii)~a second selection artifact the filter installs by defining the negative class through
attack failure, and (iv)~a preprocessing checklist.
...
The canonical stored-pair artifact for APPS control evaluations
\cite{inglis2025appscontrolarena} records \texttt{model} and \texttt{source} on every backdoor
and no generator field on any honest solution (Table~\ref{tab:schema}). Figure~\ref{fig:constructions}
sets that construction beside the one we build against it.
...
\caption{\textbf{The two constructions.} As stored, the classes differ in author, in
elicitation and in sabotage. Our control removes the first difference, the prompt-matched arm
the second. $B$ is the stored-pair baseline over $n = \baselineBn{}$ pairs; each $\rho$ is
against $B'$ refitted on that arm's own problems.}
\label{fig:constructions}
\end{figure}

\paragraph{Artifacts and provenance.}
We use seven public corpora, and their per-class provenance is the object of study: in the
APPS family the honest class is human and the attack class is one of ten named models.
\textbf{Six of the seven declare no licence} \cite{koran2026highquality,
sinha2026controlarenacorpora}, so we report only aggregates and redistribute no corpus.
...
and \nocodeCVPairedCount{} respectively (Figure~\ref{fig:monitors}, appendix): the preprocessing
...
\paragraph{The control is to have one model write both classes.}
...
\textbf{The same inflation reaches an LLM monitor.} We put a zero-shot monitor prompt to the
model that wrote both classes. It is an untrusted monitor in this field's sense, not a trusted
one, and that makes it the most favourable case: it is reading its own output. On the \emph{stored} pairs over the headline problems it scores
...
\textbf{Regenerating only the honest half makes separation worse.} With an open-weight
model writing the honest class against the stored backdoors, $\rho = \armOneRho{}$
...
In five agentic settings where the same three models write both classes, so no authorship
confound is possible, the surviving floor is structural: \bashFloor{} (\texttt{bash}, on its
...
monitor, bounds how much of the separability is content rather than partitioning it. The arm is
not bit-reproducible: it ran through a metered API that does not expose sampling parameters,
with thinking off although the stored attacker was a thinking model. The two generators are
compared on the \texttt{passes} eligibility alone, Haiku 4.5's strict row having been refused
at \haikuUsable{} pairs against our 50-pair floor.

\textbf{Scope, provenance and the harness.} Whether stored-pair corpora beyond these leak the
same way is untested: five of our seven corpora are one user's repackaging, and our claims are scoped to
...
\item \textbf{Generate both rollouts from one model under one prompt framing, and verify both
      with one harness.} If the attack class is model-written, the honest class must be too;
      regenerating one side substitutes one authorship gap for another. Matched construction is
      precedented in code-detection benchmarks \cite{guo2025codemirage}, but we are not aware
      of it reaching backdoor detection.
...
\item \textbf{Do not define the negative class by attack failure.} Drop the problems whose
      attacks failed, or sample the negative class independently of attack success.
\item \textbf{State the split, the metric and the preprocessing whenever monitors are
      compared, and release all three.} Ours are MIT (appendix~\ref{app:release}).
\end{enumerate}

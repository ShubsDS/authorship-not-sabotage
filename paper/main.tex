...
\paragraph{Three checks on the carrier.}
...
share. It is also nearly as strong on \textbf{inert} backdoors
...
model that wrote both classes. In this field's vocabulary it is an untrusted monitor, which
...
Labelling by \texttt{backdoor\_works} defines the negative class at the problem level by
\emph{attack failure}; the criterion should be honesty. \inertShare{} of the test split's labelled
...
\paragraph{A floor survives matched authorship.}
...
\textbf{Scope, provenance and the harness.} We have not tested stored-pair corpora beyond
these for the same leak. Five of our seven corpora are one user's repackaging, and our claims are
...
fire, below the 50-pair floor \texttt{gate\_s\_samegen.py} requires, leaving it with no

...
the pair; a bar spanning both columns means the two sides are identical in it. We have one model
write both classes (middle), then elicit both with the same prompt, which differs only in the
clause asking for a flaw (right); the backdoor row never merges. $B$ is the stored-pair baseline over
$n = \baselineBn{}$ pairs and $S$ the same classifier on each arm; $\rho$ is the share of
above-chance separability that survives, against $B'$ refitted on that arm's own problems.}

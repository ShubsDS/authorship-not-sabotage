...
\newcommand{\haikuUsable}{35}          % usable attacks over the 1,444-problem pool, one draw each:
                                       % below the 50-pair floor, so there is no strict row
...
$\rho = \haikuRho{}$ (\haikuRhoLo{}--\haikuRhoHi{}) on an attack class \haikuInert{} inert
against the first generator's \headlineInert{}. The first
...
separate from each other at \promptOnly{}. On the second generator neither moves: $S$ with the
prompt matched too is \haikuSMatched{} ($\rho = \haikuMatchedRho{}$) and its prompt-only floor is
\haikuPromptFloor{}, an interval containing chance. The pairs are independent programs (Table~\ref{tab:similarity}), and at
...
quantity computed two ways: the pooled points are \matchedRhoPooled{} (prompt-matched headline),
...
identifier normalisation removed is what prompt matching removes. That masked row is the one that decides whether the residue is structure or naming, and under
...
visible in the programs: the layout vocabulary the first generator's residue is made of ---
\texttt{def main}, \texttt{sys.stdin}, \texttt{data[} --- is present in \haikuDefMain{} of the
...
compared rows are not equally inert either: \haikuFireInRow{} of the \haikuN{} attacks in the
...
its own honest program failed the tests and which Gate S therefore drops. The

...
survives on attacks that actually fire is consistent with surface content a monitor also
reads. A zero-shot monitor is inflated the same way, scoring \monStoredStrictAbs{} on stored
pairs and \monitorStrict{} on firing backdoors from the same author. We recommend that
...
and the edit arm bounds $S$ under near-copies (appendix~\ref{app:robust}). A monitor is inflated the same way. A zero-shot monitor
prompt, answered by the model that wrote both classes, scores \monStoredHead{} on the
\emph{stored} pairs over the same problems and \monitorS{} (\monitorSLo{}--\monitorSHi{}) on the headline pairs, a
...
same two ratios (\monHaikuRhoHead{}, a bound at half missingness, and
\monHaikuRhoStrict{}). Classifier and monitor are
...
Figure~\ref{fig:monitors}'s ranking is a statement about those monitors on that split, not a
claim that a bag of words beats a monitor here.
...
scores in Figure~\ref{fig:monitors} are \texttt{runs/x0/monitor-summary.json} of
\cite{jha2026decorrelation}'s MIT release, used under its notice. Two interpreter-era artifacts in the upstream solutions fall on opposite classes: a

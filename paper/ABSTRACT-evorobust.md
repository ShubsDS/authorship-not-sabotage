...
**Decided 2026-09-13:** the title is **A Same-Generator Corpus and Classifier Floors for AI-Control Monitor Evaluation**. Both contributions lead and the authorship confound is left to the abstract, which opens on it; naming the confound first read as though the corpus and the floors were an afterthought. "Classifier" separates our two fitted floors from the `--leak-check` detectors, which comment stripping already drives to chance. Rejected on the way: *leakage floors* ("leakage" is not the body's word), *stored-pair floors* (collides with the stored-pair baseline $B$ in the F1 caption) and *corpus floors* (repeats the title's first noun). The Scope sentence in §1 still carries the narrowing to stored pairs that the title does not.

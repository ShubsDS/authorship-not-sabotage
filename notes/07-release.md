...
> **Updated 2026-09-13:** the paper was renamed to *A Same-Generator Corpus and Classifier
> Floors for AI-Control Monitor Evaluation*. `LICENSE` and the `RELEASE-CARD.md`
> citation block now carry that title (BibTeX key `samegeneratorcorpus`); the procedure above is
> otherwise unchanged. Any release bundle built before that date carries an older title and
> should be rebuilt.

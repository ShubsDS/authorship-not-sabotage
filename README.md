...
> 1a. **The decomposition, after §13–§15** (every value re-derived from the committed jsons by
>    [`notes/10-ledger-recheck.md`](notes/10-ledger-recheck.md), which is the block the paper
>    pastes). Matching the attack prompt's framing as well as the
>    model takes the Sonnet headline row to S = 0.6423 (n = 586), **ρ = 0.394 (0.326–0.454,
>    joint)** — partial, content excluded (P = 0.000), with the collapse line touched at the 3.3%
>    tail — and two *correct* honest classes under the two
...
>    an interval that does not touch Sonnet's (§15). The two compared rows are not equally inert:
>    2 of Haiku's 611 attacks fire (99.67% inert) against 14 of Sonnet's 623 (97.75%), and the
>    mechanism is legible — Haiku writes `input(` in every program of all three classes and
>    `def main` / `sys.stdin` / `data[` in none, so the layout the Sonnet residual is made of is
>    absent. The monitors move the same way: the same
...
| Gate S — the prompt-matched honest class (arm 3) | ✅ **Run 2026-09-11.** Model *and* prompt held constant: the honest class regenerated under the attack prompt's framing minus the flaw sentence. Headline **S = 0.6423** (0.6179–0.6621, n = 586) vs B′ = 0.8613, **ρ = 0.394** (0.326–0.454, joint) — partial, content excluded (P = 0.000), the collapse line touched at the 3.3% tail — where the same 586 problems with the original honest class give ρ = 0.712. **Prompt-only floor 0.6006** (0.5768–0.6243): two correct classes from one model under the two prompts, no attack on either side. Strict S = 0.7113 (n = 96), ρ = 0.697 (0.494–0.856): on backdoors that fire, the residue survives prompt matching. [`experiments/RESULTS.md`](experiments/RESULTS.md) §13 |
...
| Gate S — a second same-generator model | ✅ **Run 2026-09-11, and it collapses.** Claude Haiku 4.5 writes both classes through the identical pipeline: **S = 0.5340** (0.5159–0.5531, n = 611) vs B′ = 0.8584, **ρ = 0.095** (0.044–0.150, joint), P(ρ ≤ 0.33) = 1.000 — the collapse band, an interval nowhere near Sonnet's 0.625–0.755. Not bought with near-copies: S stays 0.474–0.514 in every pair-similarity stratum while B′ holds 0.788–0.833. Prompt-matched S 0.5399 (ρ = 0.113); prompt-only floor 0.4918, an interval containing chance. **No strict row**: 35 usable attacks after one draw and 59 after a second ($4.81, §15.8), but only **9** of them on problems whose honest program passes — the usable rate is 44.2% where the honest reference is itself wrong (divergence by being *right*) against 1.1% where it is sound — so the two generators are compared on the `passes` eligibility only — and not on equally inert rows either, 2 of the 611 firing (99.67% inert) against Sonnet's 14 of 623 (97.75%), the other 33 sitting on problems whose Haiku honest program failed. Mechanism: `def main` / `sys.stdin` / `data[` appear in **none** of Haiku's programs in any of the three classes, `input(` in all of them. [`experiments/RESULTS.md`](experiments/RESULTS.md) §15 |
